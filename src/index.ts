...
async function makeOpenMeteoRequest<T>(url: string): Promise<T | null> {
  try {
...
function formatOpenMeteoForecast(
...
server.registerTool(
  ...
  async ({ latitude, longitude }) => {
    ...
    if (!pointsData) {
      ...
      const openMeteoData = await makeOpenMeteoRequest<OpenMeteoResponse>(openMeteoUrl);
      if (!openMeteoData) {
        return {
          content: [
            {
